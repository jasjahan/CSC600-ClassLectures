    ...
    return (exports, Map) => {
        ...
        const csc600_RecordReference = (x, v) => {
            if (trace.length > 0) {
                ...
                trace.push(Map({x: ["ref", objectId(v), v]}));
            }
            return v;
        }
...
export const traceMemory = (f: Function, exports? /* for Jupyter */, verbose=false): MemoryTrace => {
...
export function cytoscapifyMemTrace(valRefTrace: Map<string, any>, refId: WeakMap<object, number>): string {
    let count = 0;
    function fresh(prefix: string): string {
        count += 1;
        return prefix + count;
    ...
    for (let [key, value] of valRefTrace) {
        const kind = value[0]
          if (kind == "val") {
              ...
              acc.push({ data: {id: varId, label: key}});
              const valId = fresh("val");
              acc.push({ data: {id: valId, label: value[1]}});
              ...
              console.log(value[2])
              acc.push({ data: {id: refId, label: "ref" + value[1]}});
              const objId = fresh("obj");
              acc.push({ data: {id: objId, label: value[2].toString()}});
